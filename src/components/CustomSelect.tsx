...
export function CustomSelect({
  ...
  // Normalizamos para que no falle por "1" vs 1 y para soportar ""
  ...
  return (
    <div className="relative">
      <Listbox
        value={v}
        onChange={(val: string) => {
          // si eligió placeholder (no debería venir desde options, pero por seguridad)
          if (val === "") return onChange("");

          // devolvemos el mismo tipo que en options si existe
      ...
      >
        {({ open }) => (
          <>
            <ListboxButton className="relative w-full cursor-pointer rounded-xl bg-white py-3 pl-4 pr-10 text-left border border-[#E6E1D6] shadow-sm hover:border-[#5B732E] focus:outline-none focus:ring-2 focus:ring-[#5B732E]/20 focus:border-[#5B732E] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                  ...
                  >
                    {({ selected }) => (
                      <div className="flex items-center justify-between">
                        ...
                        {selected && <Check className="h-4 w-4 text-[#6F8C1F] flex-shrink-0 ml-3" aria-hidden="true" />}
                      </div>
                    )}
                  </ListboxOption>
                );
