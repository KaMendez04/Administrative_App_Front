...
export default function SpendList() {
  ...
  if (q.loading) return <p className="text-sm text-gray-500">Cargando egresos…</p>;
  if (q.error) return <p className="text-sm text-red-600">{q.error}</p>;

  return (
    <>
      <div className="rounded-2xl border border-gray-100 bg-white p-4">
        <h3 className="mb-3 text-sm font-semibold text-gray-800">Egresos registrados</h3>
                      ...
                      <button
                        ...
                        onClick={() =>
                          setEditing({
                            ...
                            spendTypeId: sub?.spendTypeId,
                            spendSubTypeId: sub?.id,
                          })
                        }
                        title="Editar"
                      >
      ...
      <EditSpendModal open={!!editing} onClose={() => setEditing(null)} spend={editing} />
    </>
  );
}
