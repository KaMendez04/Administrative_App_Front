...
import EditExtraordinaryModal from "./editExtraordinaryModal"
...
export default function ExtraordinayList({ list, loading, onSaved }: ExtraordinayListProps) {
  const columnHelper = createColumnHelper<Extraordinary>()
  const [editing, setEditing] = useState<Extraordinary | null>(null)

  const columns: ColumnDef<Extraordinary, any>[] = useMemo(() => {
    return [
      ...
      columnHelper.display({
        ...
        cell: (info) => (
          <div className="flex justify-center">
            <ActionButtons showEdit onEdit={() => setEditing(info.row.original)} />
          </div>
        ),
      }),
    ]
