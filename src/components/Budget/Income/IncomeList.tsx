...
export default function IncomeList() {
  ...
  if (q.loading) return <p className="text-sm text-gray-500">Cargando ingresos…</p>;
  if (q.error) return <p className="text-sm text-red-600">{q.error}</p>;

  return (
    <>
      <div className="rounded-2xl border border-gray-100 bg-white p-4">
        <h3 className="mb-3 text-sm font-semibold text-gray-800">Ingresos registrados</h3>
            ...
            <tbody>
              {(q.data ?? []).map((row: any) => (
                <tr key={row.id} className="border-b last:border-b-0">
                  ...
                  <td className="py-2">{row?.incomeSubType?.name ?? "-"}</td>
                    ...
                    <button
                      ...
                      onClick={() =>
                        setEditing({
                          ...
                          amount: row.amount,
                          date: row.date,
                          incomeTypeId: row?.incomeSubType?.incomeType?.id ?? null,
                          incomeSubTypeId: row?.incomeSubType?.id,
                        })
                      }
                      title="Editar"
                    >
      ...
      <EditIncomeModal open={!!editing} onClose={() => setEditing(null)} income={editing} />
    </>
  );
}
