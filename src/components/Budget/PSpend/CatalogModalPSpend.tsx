...
export default function CatalogModalPSpend({
  open, onClose, defaultDepartmentId, defaultPSpendTypeId, onAccept,
}: Props) {
  ...
  useEffect(() => { if (defaultDepartmentId) setDepartmentId(defaultDepartmentId); }, [defaultDepartmentId]);
  useEffect(() => { if (defaultPSpendTypeId) setTypeId(defaultPSpendTypeId); }, [defaultPSpendTypeId]);
  ...
  useEffect(() => {
    if (typeof departmentId === "number" && typeId === "" && (types.data?.length ?? 0) > 0) setTypeId(types.data[0].id);
  ...
  async function handleCreateSubType() {
    ...
    try {
      await mCreateSub.mutate({ name: newSubType.trim(), pSpendTypeId: Number(typeId) });
      setNewSubType("");
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <div className="w-full max-w-3xl rounded-2xl bg-white shadow-2xl ring-1 ring-gray-100">
        ...
        <div className="grid gap-6 p-4 md:p-6">
          ...
          <section className="grid gap-2">
            ...
            <div className="flex gap-2">
              <select className="flex-1 rounded-xl border border-gray-200 px-3 py-2"
              ...
              <input className="w-64 rounded-xl border border-gray-200 px-3 py-2"
                placeholder="Nuevo departamento" value={newDepartment} onChange={(e) => setNewDepartment(e.target.value)} />
          ...
          <section className="grid gap-2">
            ...
            <div className="flex gap-2">
              <select className="flex-1 rounded-xl border border-gray-200 px-3 py-2"
                value={typeId} onChange={(e) => setTypeId(e.target.value ? Number(e.target.value) : "")} disabled={!departmentId}>
                <option value="">Seleccione…</option>
                {typeOptions.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
              </select>

              <input className="w-64 rounded-xl border border-gray-200 px-3 py-2"
                placeholder="Nuevo tipo" value={newType} onChange={(e) => setNewType(e.target.value)} disabled={!departmentId} />
              <button type="button" onClick={handleCreateType}
          ...
          <section className="grid gap-2">
            ...
            <div className="flex gap-2">
              <input className="flex-1 rounded-xl border border-gray-200 px-3 py-2"
                placeholder="Nuevo subtipo" value={newSubType} onChange={(e) => setNewSubType(e.target.value)} disabled={!typeId} />
              <button type="button" onClick={handleCreateSubType}
        ...
        <div className="flex items-center justify-end gap-3 border-t p-4 md:p-5">
          <button onClick={onClose} className="rounded-xl border border-gray-200 px-4 py-2 text-gray-700 hover:bg-gray-50">Cancelar</button>
          <button
